<script lang="ts">
  ...
  const sizing = "size-10 md:size-[10vw] md:text-[2vw]";
</script>
...
>
  {#snippet children({ months, weekdays })}
    <RangeCalendar.Header class="flex items-center justify-between md:pb-10">
      <RangeCalendar.PrevButton class={buttonClasses}>
        <CaretLeft class="size-6" />
      </RangeCalendar.PrevButton>
      <RangeCalendar.Heading class="text-[2.5vw] font-medium" />
      <RangeCalendar.NextButton class={buttonClasses}>
          ...
          <RangeCalendar.GridHead>
            <RangeCalendar.GridRow class="mb-1 flex w-full justify-between">
              {#each weekdays as day (day)}
                <RangeCalendar.HeadCell class={cn("text-muted-foreground font-normal! rounded-md", sizing)} >
              ...
              <RangeCalendar.GridRow class="flex w-full">
                {#each weekDates as date, d (d)}
                  <RangeCalendar.Cell
                    {date}
                    month={month.value}
                    class={cn("p-0! relative m-0 text-center text-sm focus-within:z-20", sizing)}
                  >
                    <RangeCalendar.Day
                      class={cn(
                    ...
                    >
                      <div
                        class="bg-foreground group-data-selected:bg-background group-data-today:block absolute top-[5px] hidden size-[1vw] rounded-full"
                      ></div>
                      <span class="md:text-[2vw]">
	                      {date.day}
                      </span>
                    </RangeCalendar.Day>
                  </RangeCalendar.Cell>
