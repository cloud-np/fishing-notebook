<script lang="ts">
	...
	async function handleSubmit(event: Event) {
		...
		try {
			...
			if (error) {
				submitError = error.message || `Failed to ${tripId ? 'update' : 'create'} trip`;
				return;
			}
...
{#if isLoading}
	<div class="trip-form-container">
		<div class="flex items-center justify-center p-8">
			<p class="text-muted-foreground">Loading trip data...</p>
		</div>
	</div>
{:else}
	<div class="trip-form-container">
		...
		<h2>{tripId ? 'Edit' : 'Create'} Fishing Trip</h2>
		...
		<div class="trip-form">
			...
			{#if submitSuccess}
				<div class="message success-message">
					Fishing trip {tripId ? 'updated' : 'created'} successfully!
				</div>
			{/if}
