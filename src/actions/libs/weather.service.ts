...
export class WeatherService {
	...
	async getHourlyWeather(date: string, latitude: number, longitude: number): Promise<OpenMeteoHourlyWeatherResponse> {
		...
		// Open-Meteo forecast API supports dates from ~5 days in the past to 16 days in the future
		// For dates older than 5 days in the past, use the historical archive API
		// For dates more than 16 days in the future, throw an error
		if (daysDiff < -16) {
			throw new Error(
				`Cannot fetch weather data for ${date}. The forecast API only supports dates up to 16 days in the future.`
			);
		}
		const useHistoricalAPI = daysDiff > 5;
		const baseUrl = useHistoricalAPI
	...
	async fetchAndSaveHourlyWeather(date: string, latitude: number, longitude: number) {
		try {
			...
			console.error("Error saving hourly weather records:", e);
		}
	}
}
